...
$id = $_GET['id'];
$data = mysqli_query($connection,"SELECT * FROM target where id='$id'");

if (isset($_POST['update'])) {
	$target = filter_input(INPUT_POST, 'target', FILTER_SANITIZE_STRING);

	// $sql = $connection->prepare("UPDATE target SET target=? WHERE id='$id'");
    // $sql->bind_param("ssss", $target);
    $sql = mysqli_query($connection,"UPDATE target SET target=$target WHERE id='$id'");
	if ($sql) {
		echo '<script>alert("Data Berhasil Disimpan"); window.location = "target.php"</script>;';
	}else{
      ...
      <div class="w-full container mx-auto flex flex-wrap items-center mt-0 pt-3 pb-3 md:pb-0">
        ...
        <div class="w-full flex-grow lg:flex lg:items-center lg:w-auto hidden lg:block mt-2 lg:mt-0 bg-white z-20" id="nav-content">
          <ul class="list-reset lg:flex flex-1 items-center px-4 md:px-0">
            ...
            <li class="mr-6 my-2 md:my-0">
              <a href="#" class="block py-1 md:py-3 pl-1 align-middle text-gray-500 no-underline hover:text-gray-900 border-b-2 border-white hover:border-yellow-500">
          ...
          <div class="w-full md:w-1/2 xl:w-1/2 p-3">
            <div class="bg-white border rounded shadow p-2">
              ...
              <form action="" method="post" class="bg-white rounded p-10">
                ...
                <input disabled id="id_target" autocomplete=off type="hidden" class="mt-1 inline-block bg-gray-200 focus:border-green-500 focus:border-6 shadow-sm appearance-none border rounded w-full py-2 px-3 text-sm text-gray-700 leading-normal focus:outline-none mb-4"  name="no_sertifikat" value="<?=$d['id'];?>"/>

								<label for="username" class="flex flex-wrap text-sm font-bold text-gray-500 leading-tight">Target</label>
                <input id="username" autocomplete=off type="text" class="mt-1 inline-block bg-gray-200 focus:border-green-500 focus:border-6 shadow-sm appearance-none border rounded w-full py-2 px-3 text-sm text-gray-700 leading-normal focus:outline-none mb-4"  name="target" value="<?=$d['target'];?>"/>
