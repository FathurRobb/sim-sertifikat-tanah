...
if(isset($_GET['submit'])){
  $no_sertifikat = filter_input(INPUT_GET, 'no_sertifikat', FILTER_SANITIZE_STRING);
  $desa = filter_input(INPUT_GET, 'desa', FILTER_SANITIZE_STRING);
  $data = mysqli_query($connection, "SELECT * FROM data_sertifikat WHERE no_sertifikat LIKE '%".$no_sertifikat."%' AND desa LIKE '%".$desa."%'");
  $judul = "HASIL PENCARIAN";
}else{
  $data = mysqli_query($connection, "SELECT * FROM data_sertifikat");
          ...
          <div class="w-full md:w-1/2 xl:w-1/2 p-3">
            <div class="bg-white border rounded shadow p-2">
              <form action="" method="GET" class="bg-white rounded p-10">
                <li class="list-none"><span class="text-sm text-gray-500">Data Sertifikat > Pencarian Data</span></li>
                <li class="list-none"><span class="text-xl font-bold">Cari Data Sertifikat</span></li>
                <input autocomplete=off type="text" class="mt-3 inline-block bg-gray-200 focus:border-green-500 focus:border-6 shadow-sm appearance-none border rounded w-full py-2 px-3 text-sm text-gray-700 leading-normal focus:outline-none mb-4" placeholder="No Sertifikat" name="sertifikat"/>
            ...
            <table class="table-auto w-full">
              ...
              ?>
                <tr class="text-center">
                  <td class="border px-4 py-2"><?=$d['no_sertifikat'];?></td>
                  <td class="border px-4 py-2"><?=$d['nama'];?></td>
                  <td class="border px-4 py-2"><?=$d['desa'];?></td>
                  <td class="border px-4 py-2"><?=$d['tahun'];?></td>
                  <td class="border px-4 py-2">
                    <a onClick="return confirm('Yakin Ingin Menghapus Data?')" href="controller/DeleteSertiController.php?no_sertifikat=<?=$d['no_sertifikat'];?>">
                    <button  title="Hapus" class="bg-red-400 px-2 py-1 text-sm rounded-md text-red-700"> <i class="fa fa-trash fa-fw"></i> </button>
                  </a>
                    <a href="edit_serti.php?no_sertifikat=<?=$d['no_sertifikat'];?>"><button title="Edit" class="bg-green-400 px-2 py-1 text-sm rounded-md text-green-700"> <i class="fa fa-edit fa-fw"></i> </button></a>
                  </td>
                </tr>
              <!---MODAL--->
