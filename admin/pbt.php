...
if(isset($_GET['submit'])){
  $no_pbt = filter_input(INPUT_GET, 'no_pbt', FILTER_SANITIZE_STRING);
  $desa = filter_input(INPUT_GET, 'desa', FILTER_SANITIZE_STRING);
  $data = mysqli_query($connection, "SELECT * FROM data_pbt WHERE no_pbt LIKE '%".$no_pbt."%' AND desa LIKE '%".$desa."%'");
  $judul = "HASIL PENCARIAN";
}else{
  $data = mysqli_query($connection, "SELECT * FROM data_pbt");
      ...
      <div class="w-full container mx-auto flex flex-wrap items-center mt-0 pt-3 pb-3 md:pb-0">
        ...
        <div class="w-1/2 pr-0">
          <div class="flex relative inline-block float-right">
            <div class="relative text-sm">
              <button id="userButton" class="flex items-center focus:outline-none mr-3"> <span class="hidden md:inline-block">Hi, <?php echo $_SESSION['username'];?></span>
        ...
        <div class="w-full flex-grow lg:flex lg:items-center lg:w-auto hidden lg:block mt-2 lg:mt-0 bg-white z-20" id="nav-content">
          <ul class="list-reset lg:flex flex-1 items-center px-4 md:px-0">
            ...
            <li class="mr-6 my-2 md:my-0">
              <a href="pbt.php" class="block py-1 md:py-3 pl-1 align-middle text-green-500 no-underline hover:text-gray-900 border-b-2 border-green-500 hover:border-green-500">
          ...
          <div class="w-full md:w-1/2 xl:w-1/2 p-3">
            <div class="bg-white border rounded shadow p-2">
              <form action="" method="GET" class="bg-white rounded p-10">
                <li class="list-none"><span class="text-sm text-gray-500">Data PBT > Pencarian Data</span></li>
                <li class="list-none"><span class="text-xl font-bold">Cari Data PBT</span></li>
                <input autocomplete=off type="text" class="mt-3 inline-block bg-gray-200 focus:border-green-500 focus:border-6 shadow-sm appearance-none border rounded w-full py-2 px-3 text-sm text-gray-700 leading-normal focus:outline-none mb-4" placeholder="No PBT" name="no_pbt"/>
            ...
            <table class="table-auto w-full">
              ...
              ?>
                <tr class="text-center">
                  <td class="border px-4 py-2"><?=$d['no_pbt'];?></td>
                  <td class="border px-4 py-2"><?=$d['nama'];?></td>
                  <td class="border px-4 py-2"><?=$d['desa'];?></td>
                  <td class="border px-4 py-2"><?=$d['tahun'];?></td>
                  <td class="border px-4 py-2"><?=$d['dokumen'];?></td>
                  <td class="border px-4 py-2">
                    <a onClick="return confirm('Yakin Ingin Menghapus Data?')" href="controller/DeletePBTController.php?no_pbt=<?=$d['no_pbt'];?>">
                    <button  title="Hapus" class="bg-red-400 px-2 py-1 text-sm rounded-md text-red-700"> <i class="fa fa-trash fa-fw"></i> </button>
                  </a>
                    <a href="edit_pbt.php?no_pbt=<?=$d['no_pbt'];?>"><button title="Edit" class="bg-green-400 px-2 py-1 text-sm rounded-md text-green-700"> <i class="fa fa-edit fa-fw"></i> </button></a>
                  </td>
                </tr>
              <!---MODAL--->
                ...
                <div class="modal-container bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg z-50 overflow-y-auto">
                  ...
                  <div class="modal-content py-4 text-left px-6">
                    ...
                    <div class="flex justify-end pt-2">
                      <a href="controller/DeleteSertiController.php?no_sertifikat=<?=$d['no_sertifikat'];?>"><button class="px-4 bg-transparent p-3 rounded-lg text-indigo-500 hover:bg-gray-100 hover:text-indigo-400 mr-2">Hapus</button></a>
